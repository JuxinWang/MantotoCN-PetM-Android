...
public class NotificationActivity extends BaseActivity implements OnClickListener{
    ...
    protected void loadDatas() {
        ...
        try {
            object.put("keeperid", 0);
        } catch (JSONException e) {
            e.printStackTrace();
        }
        IRequest.postJson(NotificationActivity.this, Constant.REMIND_GET_ALL, object, new RequestListener() {
            @Override
            public void requestSuccess(JSONObject json) {
                ...
                if (workOrders.code == 200){
                    if (workOrders.data.size()>0){
                        mAdapter = new RemindAdapter(NotificationActivity.this,workOrders.data);
                        remindRevyvler.setAdapter(mAdapter);
                    }
                }else if(workOrders.code == 201){
                    noRmind.setVisibility(View.VISIBLE);
                }else {
                    ToastU.showShort(NotificationActivity.this,workOrders.desc);
                }
            }
